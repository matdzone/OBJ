...
function drawTank(player: any): void {
    ...
    context.lineTo(
        centerX + tank.directionX * 10,
        centerY + tank.directionY * 10
    );
...
function drawProjectile(projectile: any): void {

    const centerX =
        projectile.position.x * tileSize +
        tileSize / 2;

    const centerY =
        projectile.position.y * tileSize +
        tileSize / 2;
...
async function updateGame(): Promise<void> {
    ...
    const projectileResponse =
        await fetch(`${serverUrl}/projectiles`);

    const projectiles =
        await projectileResponse.json();
